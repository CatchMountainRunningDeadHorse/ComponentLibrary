...
import { getFormList, getMyApplyData, GeneratePdf } from '@/api/query'
...
const handleNodeClick = (data) => {
  ...
  Cookies.set('FlowID', data.FlowID)
  Cookies.set('activeName', activeName.value)
  Cookies.set('flowName', data.FlowName)
  getTableData()
}
const total = ref(400)
...
const getTableData = async () => {
  const { data, PageTotalNums } = await getMyApplyData(queryForm.value)
  data.forEach((item) => {
    item.ApplyStatuStr = statuString(item.ApplyStatus)
  })
  tableData.value = data
  total.value = PageTotalNums * queryForm.value.pageSize
}
